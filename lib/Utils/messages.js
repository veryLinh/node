...
const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === "audio" && uploadData.ptt === true) {
        uploadData.media = await ensurePttOpus(uploadData.media, logger);
    ...
    if (isNewsletter) {
        ...
        if (mediaType === "audio" && uploadData.ptt === true) {
            try {
                ...
                if (typeof uploadData.waveform === "undefined") {
                    uploadData.waveform = await getAudioWaveform(filePath, logger);
                }
            } catch (error) {
                logger?.warn(
                    { trace: error.stack },
                    "failed to compute ptt audio duration/waveform for newsletter"
                );
            }
        }
    ...
    const isRemoteVideoUrl =
        typeof uploadData.media === "object" &&
        uploadData.media !== null &&
        "url" in uploadData.media &&
        /^https?:\/\//i.test(uploadData.media.url?.toString() || "");
    if (mediaType === "video" && !isRemoteVideoUrl) {
        uploadData.media = await ensureFaststartVideo(uploadData.media, logger);
    }
    const {
        mediaKey: mediaKey,
...
const buildProtoContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, "text")) {
            ...
            viewOnceMessage: {
                message: {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        footer: {
                            text: os.footer || "Powered by vellia"
                        },
                        nativeFlowMessage: {
                            buttons: [
                                {
        ...
        if (rich.images) {
            ...
            submessages.push({
                ...
                gridImageMetadata: {
                    ...
                    imageUrls: urls.map(url => ({ imagePreviewUrl: url, imageHighResUrl: url, sourceUrl: 'https://www.vellia.cloud/' }))
                }
            });
            urls.forEach(url => {
                sections.push({
        ...
        if (rich.video) {
            submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_VIDEO - Vellia Elyvia ]' });
            sections.push({
                view_model: {
                    primitive: {
        ...
        if (rich.productSingle) {
            submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_PRODUCT - vellia ]' });
            sections.push({
                view_model: {
        ...
        if (rich.post) {
            submessages.push({ messageType: 2, messageText: '[ CANNOT_LOAD_POST - vellia ]' });
        ...
        const useUnified = rich.autoUnified === true || !!rich.unified;
            ...
            botForwardedMessage: {
                message: {
                    richResponseMessage: {
                        ...
                        ...(useUnified ? {
                            unifiedResponse: {
                                data: rich.unified || Buffer.from(JSON.stringify(unifiedData)).toString('base64')
                            }
                        } : {}),
                        contextInfo: {
                            forwardingScore: 1,
