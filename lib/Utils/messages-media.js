...
const ensurePttOpus = async (media, logger) => {
    const { stream: rawStream } = await getStream(media);
    const buffer = await toBuffer(rawStream);
    try {
        let metadata;
        try {
        ...
        if (codec.includes("opus") && container.includes("ogg")) {
            logger?.debug("ptt audio already opus, skipping transcode");
            return buffer;
        }
        const tmpIn = join(getTmpFilesDirectory(), `${generateMessageID()}-ptt-in`);
        const tmpOut = join(getTmpFilesDirectory(), `${generateMessageID()}-ptt-out.ogg`);
        await promises.writeFile(tmpIn, buffer);
        try {
            await new Promise((resolve, reject) => {
                const cmd = `ffmpeg -y -i "${tmpIn}" -c:a libopus -ar 48000 -ac 1 -b:a 64k -vn -f ogg "${tmpOut}"`;
                exec(cmd, (err) => {
                    if (err) {
                        reject(err);
                    } else {
            ...
            const converted = await promises.readFile(tmpOut);
            logger?.debug("transcoded ptt audio to opus");
            return converted;
        } finally {
            await promises.unlink(tmpIn).catch(() => {});
            await promises.unlink(tmpOut).catch(() => {});
        }
    } catch (e) {
        logger?.warn(
            { trace: e.stack },
            "failed to transcode ptt audio to opus, sending original buffer as-is"
        );
        return buffer;
    }
};
const ensureFaststartVideo = async (media, logger) => {
    const { stream: rawStream } = await getStream(media);
    const buffer = await toBuffer(rawStream);
    const tmpIn = join(getTmpFilesDirectory(), `${generateMessageID()}-video-in.mp4`);
    const tmpOut = join(getTmpFilesDirectory(), `${generateMessageID()}-video-out.mp4`);
    await promises.writeFile(tmpIn, buffer);
    try {
        const codec = await new Promise((resolve, reject) => {
            const cmd = `ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of csv=p=0 "${tmpIn}"`;
            exec(cmd, (err, stdout) => {
                if (err) {
                    reject(err);
                } else {
        ...
        const isWaCompatible = codec === "h264";
        const cmd = isWaCompatible
            ? `ffmpeg -y -i "${tmpIn}" -c copy -movflags +faststart "${tmpOut}"`
            : `ffmpeg -y -i "${tmpIn}" -c:v libx264 -preset veryfast -crf 23 -pix_fmt yuv420p -c:a aac -movflags +faststart "${tmpOut}"`;
        await new Promise((resolve, reject) => {
            exec(cmd, (err, stdout, stderr) => {
                if (err) {
                    console.error(`[ensureFaststartVideo] ffmpeg FAILED: ${err.message}`);
                    console.error(`[ensureFaststartVideo] stderr: ${stderr}`);
                    reject(err);
                } else {
                    resolve();
                }
        ...
        const remuxed = await promises.readFile(tmpOut);
        if (!remuxed || remuxed.length === 0) {
            throw new Error("ffmpeg produced empty output file");
        }
        logger?.debug("processed video for WA compatibility");
        return remuxed;
    } catch (e) {
        console.error(`[ensureFaststartVideo] FALLBACK to original buffer, reason: ${e.message}`);
        logger?.warn(
            { trace: e.stack },
            "failed to process video for WA compatibility, sending original buffer as-is"
        );
        return buffer;
    } finally {
        await promises.unlink(tmpIn).catch(() => {});
...
const extractImageThumb = async (bufferOrFilePath, width = 32, quality = 50) => {
    ...
    return { buffer: buffer, original: original };
};
...
module.exports = {
    ...
    ensurePttOpus: ensurePttOpus,
    ensureFaststartVideo: ensureFaststartVideo,
