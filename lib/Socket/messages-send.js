...
const messDefaults = require('./message-flow');
...
const makeMessagesSocket = (config) => {
    ...
    const flowMsg = new messDefaults(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = flowMsg.detectType(content);
   
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await flowMsg.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await flowMsg.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await flowMsg.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await flowMsg.handleAlbum(content, jid, quoted)
                        return albumContent;
                        
                    case 'EVENT':
                        return await flowMsg.handleEvent(content, jid, quoted)
                        
                    case 'POLL_RESULT':
                        return await flowMsg.handlePollResult(content, jid, quoted)
                    
                    case 'GROUP_STORY':
                        return await flowMsg.handleGroupStory(content, jid, quoted)
                }
            }
