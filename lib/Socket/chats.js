...
const { getUrlFromDirectPath } = require("../Utils");
...
const makeChatsSocket = (config) => {
    ...
    const updateBlockStatus = async (jid, action) => {
        ...
        const itemAttrs = { action: action, jid: lid };
        if (action === "block") {
            if (!pn_jid) {
    ...
    const getBusinessProfile = async (jid) => {
        ...
        if (profiles) {
            ...
            const coverPhotoNode = getBinaryNodeChild(profiles, "cover_photo");
            let coverPhoto;
            if (coverPhotoNode) {
                const { id, direct_path, url } = coverPhotoNode.attrs;
                coverPhoto = {
                    id,
                    url: url || (direct_path ? getUrlFromDirectPath(direct_path) : undefined),
    ...
    const getBusinessCoverPhoto = async (id) => {
        const result = await query({
            tag: "iq",
    ...
    return {
        ...
        getBusinessCoverPhoto: getBusinessCoverPhoto,
